...
def preprocess(tweet, selected_text, sentiment, training=True):
    ...
    # # augmentation here (experimental)
    # if training:
    #     if np.random.random() > 0.8:
    #         random_start = np.random.choice(np.arange(0, max(1, idx_start)), size=1)[0]
    #         random_end = np.random.choice(np.arange(idx_end, max(idx_end+1, len(tweet))), size=1)[0]
    #         # define new tweet
    #         tweet = tweet[random_start:random_end]
    #         # re-find the intersection between text and selected text
    #         idx_start, idx_end = None, None
    #         for index in (i for i, c in enumerate(tweet) if c == selected_text[0]):
    #             if tweet[index:index+len(selected_text)] == selected_text:
    #                 idx_start = index
    ...
    for i in range(len(input_ids_orig)):
        if input_ids_orig[i] == 14434:
            input_ids_orig[i] = 3 # change ~ token id to <unk> token id
...
class Generator(tf.data.Dataset):
    ...
    def _generator(tweet, selected_text, sentiment, training):
        for tw, st, se in zip(tweet, selected_text, sentiment):
            yield preprocess(tw, st, se, training)

    # This dataset object will return a generator
    def __new__(cls, tweet, selected_text, sentiment, training):
        return tf.data.Dataset.from_generator(
            cls._generator,
            output_types=cls.OUTPUT_TYPES,
            args=(tweet, selected_text, sentiment, training)
        )

    @staticmethod
    def create(dataframe, batch_size, shuffle_buffer_size=-1, training=True):
        dataset = Generator(
